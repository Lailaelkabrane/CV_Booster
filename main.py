...
AZURE_VISION_DEP = os.getenv("AZURE_VISION_DEP")
...
if not AZURE_ENDPOINT or not AZURE_API_KEY:
    raise ValueError("⚠ Les variables AZURE_ENDPOINT ou AZURE_API_KEY ne sont pas définies dans .env !")
...
SECTION_HEADERS_RE = {
    ...
    "languages":      re.compile(r"^(languages?|langues?|spoken languages?|langues? parlées?|compétences? linguistiques?)$", re.I),
}
...
def calculate_ats_score(sections: dict, full_text: str,
    ...
    # 2. Technical Skills (18 pts) — semantic concept matching
    _SKILL_CONCEPTS = [
        "programming language", "database and query language",
        "machine learning and AI framework", "version control and collaboration tool",
        "containerization and deployment tool", "data visualization and BI tool",
        "cloud platform and infrastructure", "data processing and pipeline framework",
        "statistical analysis and modeling", "natural language processing",
    ]
    if SEMANTIC_OK:
        cv_sentences = [s.strip() for s in re.split(r"[\n•\-]", full_text) if len(s.strip()) > 5]
        if not cv_sentences: cv_sentences = [full_text[:500]]
        cv_embs      = _embedder.encode(cv_sentences, convert_to_tensor=True, show_progress_bar=False)
        concept_embs = _embedder.encode(_SKILL_CONCEPTS, convert_to_tensor=True, show_progress_bar=False)
        matched_concepts = []
        for i, concept in enumerate(_SKILL_CONCEPTS):
            sims = st_util.cos_sim(concept_embs[i], cv_embs)[0]
            if float(sims.max()) >= 0.40:
                matched_concepts.append(concept)
        skill_count  = len(matched_concepts)
        skill_score  = min(round(skill_count / len(_SKILL_CONCEPTS) * 18), 18)
        skill_detail = f"{skill_count}/{len(_SKILL_CONCEPTS)} skill categories covered (semantic)"
        log.info("✓ Skills semantic: %d/%d categories matched", skill_count, len(_SKILL_CONCEPTS))
    else:
        _SKILL_FALLBACK = [
        ...
        found_sk     = [s for s in _SKILL_FALLBACK if s in text_lo]
        skill_count  = len(found_sk)
        skill_score  = min(round(skill_count / 5 * 18), 18)
        skill_detail = f"{skill_count} technical skills detected (exact match)"
    scores["skills"] = {"score": skill_score, "max": 18, "detail": skill_detail}
    ...
    # 7. Professional Summary (10 pts) — semantic similarity
    ...
    _SUMMARY_REFERENCE = (
        "motivated experienced professional with technical expertise, "
        "proven results, and strong background in the field"
    )
    _SUMMARY_KWS_FALLBACK = [
    ...
    if SEMANTIC_OK and profile_text:
        ref_emb = _embedder.encode(_SUMMARY_REFERENCE, convert_to_tensor=True, show_progress_bar=False)
        sum_emb = _embedder.encode(profile_text,       convert_to_tensor=True, show_progress_bar=False)
        sim     = float(st_util.cos_sim(ref_emb, sum_emb)[0][0])
        has_summary_kws = sim >= 0.35
        log.info("✓ Summary semantic similarity: %.2f (threshold 0.35)", sim)
    else:
        has_summary_kws = any(kw in profile_text.lower() or kw in top_text
                              for kw in _SUMMARY_KWS_FALLBACK)
...
import save_cv                              # noqa: F401  (registers /extract-cv and /save-cv routes)
import enhance_for_job                     # noqa: F401  (registers /job-data and /enhance-cv-for-job routes)